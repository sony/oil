...
if __name__ == "__main__":
    ...
    config_suffix = "" if dataset == "final" else "_official"
    eval_config_path = ROOT_DIR / "data" / "env_configs" / f"eval_config_realistic{config_suffix}.json"

    advertiser_info_path = ROOT_DIR / "data" / "advertiser_info" / f"{dataset}.csv"
    advertiser_info = pd.read_csv(advertiser_info_path)
    ...
    for n in range(num_repeat):
        for i, info in advertiser_info.iterrows():
            budget = info["budget"]
            target_cpa = info["CPAConstraint"]
            advertiser = info["advertiserNumber"]
            ep_rew = 0
            step = 0
            obs, _ = env.reset(budget=budget, target_cpa=target_cpa, advertiser=advertiser, seed=n)
            done = False
            while not done:
